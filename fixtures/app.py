...
from sqlalchemy.types import Integer, Text, String, Date, DateTime, BigInteger, TIMESTAMP, Boolean, JSON
import json
...
sql = "SELECT " \
      "firstname, CONCAT(surname, ' (LIVELINK SKELETON)') as surname, createddate, type, caserecnumber, correspondencebypost, correspondencebyphone, correspondencebyemail, uid " \
      "FROM etl3.persons " \
      "WHERE sirius_id IS NULL " \
      "ORDER BY id ASC LIMIT 10"
persons_df = pd.read_sql_query(sql, con=db_engine, index_col=None)
persons_df["correspondencebypost"] = persons_df["correspondencebypost"].replace({'True': 1, 'False': 0})
persons_df["correspondencebyphone"] = persons_df["correspondencebyphone"].replace({'True': 1, 'False': 0})
persons_df["correspondencebyemail"] = persons_df["correspondencebyemail"].replace({'True': 1, 'False': 0})
...
cases_df = cases_df.drop(['id_case', 'casrec_id', 'c_order_no', 'sirius_id', 'sirius_client_id'], axis=1)
...
sql = "SELECT client_id, id FROM cases WHERE caserecnumber IN (SELECT caserecnumber FROM persons ORDER BY id DESC LIMIT 5)"
person_caseitems_df = etl_cases_df = pd.read_sql_query(sql, con=sirius_db_engine, index_col=None)
...
#     dict(
#         type='Case note',
#         description='This is a note on a skeleton client. It is to prove that the casrec migration will not overwrite the note.',
#         name='Casrec Migration validation note',
...
#
# sql = "SELECT id FROM persons ORDER BY id DESC LIMIT 10"
# persons_df = pd.read_sql_query(sql, con=sirius_db_engine, index_col=None)
#
# sql = "SELECT id FROM notes ORDER BY id DESC LIMIT 10"
# notes_df = pd.read_sql_query(sql, con=sirius_db_engine, index_col=None)
#
# DataFrames = [persons_df["id"], notes_df["id"]]
# person_note_df = pd.concat(DataFrames, axis=1, keys=["person_id", "note_id"])
# print(person_note_df)
# person_note_df.to_sql(
#     'person_note',
#     sirius_db_engine,
#     if_exists='append',
#     index=False
