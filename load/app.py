...
from sqlalchemy.types import Integer, Text, String, DateTime, Date, BigInteger, TIMESTAMP, Boolean, JSON
import json
...
print("- Addresses")
sql = "SELECT sirius_id, sirius_person_id, address_lines, town, county, postcode, isairmailrequired " \
      "FROM etl3.addresses " \
      "WHERE sirius_id IS NOT NULL "
addresses_df = pd.read_sql_query(sql, con=db_engine, index_col=None)
addresses_df["isairmailrequired"] = addresses_df["isairmailrequired"].replace({'True': 1, 'False': 0})
...
# needs an index
sql = "UPDATE etl3.addresses SET sirius_person_id = map.id FROM etl3.sirius_map_persons map WHERE map.caserecnumber = addresses.caserecnumber"
db_engine.execute(sql)

print("- Addresses")
sql = "SELECT sirius_person_id, address_lines, town, county, postcode, isairmailrequired " \
      "FROM etl3.addresses " \
      "WHERE sirius_id IS NULL"
addresses_df = pd.read_sql_query(sql, con=db_engine, index_col=None)
addresses_df["isairmailrequired"] = addresses_df["isairmailrequired"].replace({'True': 1, 'False': 0})
...
print("- Cases")
# needs an index
sql = "UPDATE etl3.cases SET sirius_client_id = map.id FROM etl3.sirius_map_persons map WHERE map.caserecnumber = cases.caserecnumber"
db_engine.execute(sql)
