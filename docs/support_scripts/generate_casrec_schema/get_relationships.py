        ...
        if "Source file" not in join_table:
            if str(":") not in str(old_join_col):
                try:
                    ...
                    unique_constraint = (
                        f"ALTER TABLE {schema_name}.{source_table.lower()} "
                    ...
                    statement = (
                        f"ALTER TABLE {schema_name}.{join_table.lower()} \r\n"
...
[all_unique_constraints.append(x) for x in uniques if x not in all_unique_constraints]
all_fk_constraints = []
[all_fk_constraints.append(x) for x in fks if x not in all_fk_constraints]
