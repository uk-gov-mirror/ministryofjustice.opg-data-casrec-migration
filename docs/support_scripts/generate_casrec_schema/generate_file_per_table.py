...
table_names = df
table_names['counter'] = (table_names['Unnamed: 0'].astype(str).str[:12] == "Logical File").cumsum()
table_names_2 = pd.DataFrame()
table_names_2['table_number'] = table_names['counter']
table_names_2['table_name'] = table_names['Unnamed: 0'].apply(lambda x: str(x)[14:]  if str(x)[:12] =="Logical File" else 'row')
...
for idx, group in df.groupby(df['counter']):
    ...
    table_name = table_names_2.loc[table_names_2['table_number'] == idx,
                              'table_name'].values[0]
    ...
    table_with_types = pd.merge(table_df, mapping, left_on="Type", right_on="code",
                      how="left")
