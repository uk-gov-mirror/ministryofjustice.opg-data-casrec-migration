...
def main():
    ...
    if environment in ("local", "development"):
        ...
        print("- Persons")
        sql = """UPDATE acquire_target_ids.persons persons
        ...
        # 2 = hw
        sql = """UPDATE acquire_target_ids.cases cases
        SET ordersubtype = 'hw'
        WHERE ordersubtype = '2';
        UPDATE acquire_target_ids.sirius_map_cases SET sirius_casesubtype = 'hw' WHERE sirius_casesubtype = '2'"""
        etl3_db_engine.execute(sql)
        sql = """UPDATE acquire_target_ids.cases cases
        SET ordersubtype = 'pfa'
        WHERE ordersubtype != 'hw';
        UPDATE acquire_target_ids.sirius_map_cases
        SET sirius_casesubtype = 'pfa'
        WHERE sirius_casesubtype != 'hw'"""
        etl3_db_engine.execute(sql)

        sql = """UPDATE acquire_target_ids.person_caseitem
        SET sirius_person_id = persons.sirius_id
        FROM acquire_target_ids.persons WHERE persons.id = CAST(person_caseitem.person_id AS INTEGER)"""
        etl3_db_engine.execute(sql)
        sql = """UPDATE acquire_target_ids.cases cases
        SET sirius_id = map.sirius_cases_id
        FROM acquire_target_ids.person_caseitem, acquire_target_ids.sirius_map_cases map
        ...
        etl3_db_engine.execute(sql)
        sql = """UPDATE acquire_target_ids.person_caseitem
        SET sirius_case_id = acquire_target_ids.cases.sirius_id
        FROM acquire_target_ids.cases WHERE cases.id = CAST(person_caseitem.case_id AS INTEGER)"""
        etl3_db_engine.execute(sql)
        sql = """UPDATE acquire_target_ids.cases cases
        SET sirius_client_id = map.sirius_persons_id
        FROM acquire_target_ids.sirius_map_cases map WHERE map.sirius_cases_id = cases.sirius_id"""
        etl3_db_engine.execute(sql)

        print("- Addresses")
        sql = """UPDATE acquire_target_ids.addresses
        SET sirius_id = map.sirius_addresses_id
        FROM acquire_target_ids.persons persons, acquire_target_ids.sirius_map_addresses map
        WHERE persons.id = CAST(addresses.person_id AS INTEGER)
        AND map.sirius_persons_id = persons.sirius_id"""
        etl3_db_engine.execute(sql)
        sql = """UPDATE acquire_target_ids.addresses
        SET sirius_person_id = persons.sirius_id
        FROM acquire_target_ids.persons persons WHERE persons.id = CAST(addresses.person_id AS INTEGER)"""
        etl3_db_engine.execute(sql)

        print("- Notes")
        sql = """UPDATE acquire_target_ids.person_note
        SET sirius_person_id = persons.sirius_id
        FROM acquire_target_ids.persons WHERE persons.id = CAST(person_note.person_id AS INTEGER)"""
        etl3_db_engine.execute(sql)
    else:
