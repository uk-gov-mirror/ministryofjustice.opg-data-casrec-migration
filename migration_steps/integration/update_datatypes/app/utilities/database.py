...
class InsertData:
    ...
    def _create_table_statement(self, mapping_details: Dict, table_name: str) -> str:
        ...
        for col, details in mapping_details.items():
            details["data_type"] = (
                self.pg_remap[details["data_type"]]
                if details["data_type"] in self.pg_remap
    ...
    def _add_missing_columns(self, table_name, col_diff, mapping_details):
    ...
    def run_create_table_statement(self, table_name, statement):

        log.debug(f"Running create table statement: {table_name}")
        log.log(config.VERBOSE, statement)
        self.db_engine.execute(statement)

    def insert_data(self, table_name, df, mapping_details):
        ...
        if self._check_table_exists(table_name=table_name):
            ...
            if len(col_diff) > 0:
                add_missing_colums_statement = self._add_missing_columns(
                    table_name, col_diff, mapping_details
                )
                log.log(
                    config.VERBOSE,
                    f"add_missing_colums_statement: {add_missing_colums_statement}",
                )
                self.db_engine.execute(add_missing_colums_statement)
        else:
            create_table_statement = self._create_table_statement(
                table_name=table_name, mapping_details=mapping_details
            )
