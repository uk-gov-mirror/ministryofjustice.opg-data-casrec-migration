...
def generate_inserts(db_config, db_engine):
    ...
    for i, table in enumerate(tables_list):
        ...
        log.info(f"{len(completed_tables)}/{len(tables_list)} tables have been completed")
        ...
        get_source_cols_query = get_columns_query(table=table, schema=db_config['source_schema'])
        get_target_cols_query = get_columns_query(table=table, schema=db_config['target_schema'])

        source_columns = [x[0] for x in db_engine.execute(get_source_cols_query).fetchall()]
        target_columns = [x[0] for x in db_engine.execute(get_target_cols_query).fetchall()]
        columns_missing_from_target = list(set(source_columns) - set(target_columns))

        if len(columns_missing_from_target) > 0:
            alter_target_query = add_missing_columns_query(table=table, schema=db_config['target_schema'], columns=columns_missing_from_target)
        ...
        try:
            ...
            log.error(f"There was an error inserting {table} into {db_config['target_schema']}")
            break
