...
class Mapping:
    ...
    def _apply_column_alias(self, df):
        ...
        df['alias'] = df['alias'].apply(lambda x:
                                              f"{x[0]} {x[1]}"
                                              if str(x[0]) != "nan" and int(x[1]) > 0
                                              else x[0])
    ...
    def _apply_source_unique_id(self, df, source_table_name):
        df = df.append(
            {'column_name': 'casrec_id', 'casrec_table': source_table_name,
             'casrec_column_name': 'rct', 'alias': 'rct'}, ignore_index=True)
    ...
    def mapping_df(self, file_name, sheet_name, source_table_name):
        ...
        mapping_doc = self._apply_source_unique_id(df=mapping_doc,
    ...
    def mapping_definitions(self, mapping_df):
        ...
        mapping_definitions = {}
        mapping_definitions['simple_mapping'] = self.simple_mapping(full_mapping=mapping_df)
